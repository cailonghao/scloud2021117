...
public class MessageTemplate {
     ...
     * @return
     */
    public  NotificationTemplate getNotificationTemplate(Message message) {
        NotificationTemplate template = new NotificationTemplate();
        template.setAppId(appId);
        template.setAppkey(appKey);
        //设置展示样式
        template.setStyle(pushStyle.getStyle0(message));
...
//        template.setDuration("2019-07-09 11:40:00", "2019-07-09 12:24:00");  // 设置定时展示时间，安卓机型可用
        template.setNotifyid(message.getRand()); // 在消息推送的时候设置notifyid。如果需要覆盖此条消息，则下次使用相同的notifyid发一条新的消息。客户端sdk会根据notifyid进行覆盖。
     ...
     * @return
     */
    public LinkTemplate getLinkTemplate(Message message) {
        ...
        //设置展示样式
        template.setStyle(pushStyle.getStyle0(message));
        template.setUrl(message.getWebUrl());  // 设置打开的网址地址
        template.setNotifyid(message.getRand()); // 在消息推送的时候设置notifyid。如果需要覆盖此条消息，则下次使用相同的notifyid发一条新的消息。客户端sdk会根据notifyid进行覆盖。
//         template.setSmsInfo(PushSmsInfo.getSmsInfo()); //短信补量推送
//        template.setDuration("2019-07-09 11:40:00", "2019-07-09 12:24:00");  // 设置定时展示时间，安卓机型可用
        return template;
     ...
     * @return
     */
    public TransmissionTemplate getTransmissionTemplate(Message message) {
        ...
        template.setTransmissionContent(message.getContent()); //透传内容
        template.setAPNInfo(getAPNPayload()); //ios消息推送
...
//        template.setSmsInfo(PushSmsInfo.getSmsInfo()); //短信补量推送
    ...
    //点击通知, 打开（自身）应用内任意页面
    public StartActivityTemplate getStartActivityTemplate(Message message) {
